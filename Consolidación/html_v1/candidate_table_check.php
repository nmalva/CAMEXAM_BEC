...
function write_candidate($session_prc_id, $get_exa_id){
    ...
    $sql="SELECT * FROM Candidate 
                   ...
                   WHERE exa_id={$get_exa_id} AND Candidate.prc_id={$session_prc_id} AND (can_status=0 OR can_status=3)";
    $resultado=$class_bd->ejecutar($sql);
    $i=1;
    while ($r=$class_bd->retornar_fila($resultado))
    {
        ...
        if ($r["can_visa"]==0)
         ...
         $line="<tr>";
         $line.="<td>{$r["can_id"]}</td>";
         $line.="<td>{$r["can_firstname"]}</td>";
         $line.="<td>{$r["can_lastname"]}</td>";
         $line.="<td>{$r["can_dni"]}</td>";
         $line.="<td><a target='_blank' href='{$path_payment}'>{$can_paymentfile}</a></td>";
         $line.="<td><a target='_blank' href='{$path_dni}'>{$can_dnifile}</a></td>";
         $line.="<td><a target='_blank' href='{$path_aci}'>{$can_acifile}</a></td>";
         $line.="<td><a target='_blank' href='{$path_disability}'>{$can_disabilityfile}</a></td>";
         $line.="<td style='text-align: center;'>
                    
         		<select onchange='updateStatus({$r["can_id"]}, this.value);'>
    				<option  name='{$r["can_id"]}' value='0' {$checked0} {$disabled0}> Not Sent </option>
                    <option  name='{$r["can_id"]}' value='1' {$checked1} {$disabled1}> Sent </option>
                    <option  name='{$r["can_id"]}' value='2' {$checked2} disabled='disabled'> Confirmed </option>    
                    <option  name='{$r["can_id"]}' value='3' {$checked3} disabled='disabled'> Error </option>  			
    			</select>	
    			<img alt='loading' src='../../assets/admin/layout3/img/loading-spinner-blue.gif' id='update_check' style='display:none;'>

         </td>";
         $line.="<td><input type='text' onfocusout='updateComment({$r["can_id"]},this.value);' value='{$r["can_comment"]}'></td>";
         $line.="<td>{$r["can_commentadmin"]}</td>";
			...
			<div class="container">
				...
				<!-- BEGIN PAGE BREADCRUMB -->
        			<ul class="page-breadcrumb breadcrumb">
        				<li>
        					<a href="candidate_table.php?exa_id=<?php echo $get_exa_id;?>">Return</a>
        				</li>
        			</ul>
			<!-- END PAGE BREADCRUMB -->
...
function updateStatus(can_id, can_status){
	//showUpdateCheck();
	if (can_status==1){
        	mensaje =	"You are sending the candidate to be verified and registered. After this, You will not be able to modify the information. Do you want to proceed?";
        	confirmar=confirm(mensaje);    
        	if (confirmar){                
        		$.ajax({
...
function updateComment(can_id, can_comment){
	//showUpdateCheck();
	if (can_comment!=""){      //this if is to avoid the error of blank field
    	$.ajax({
            url:"../abm/abm.candidate.php",
            type: "POST",
            data:{can_id:can_id, can_comment:can_comment}, 
            success: function(opciones){ 
